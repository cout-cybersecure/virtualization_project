# Virtualization Project - Process VM Foundation

This repository now provides a complete **phase-1 process VM executable** for secure sandboxed bytecode execution, designed for infosec lab environments and multi-instance orchestration.
...
1. Optional AppArmor profile coordination on top of current fork+seccomp worker mode.
2. Capability-gated virtual devices (filesystem/network/time) with policy engine.
3. Snapshot/restore and deterministic replay for exploit repro pipelines.
4. Shared control plane adapter for future full hypervisor backend (KVM/VMX/SVM).
